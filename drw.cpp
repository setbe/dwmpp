...
void
drw_clr_create_rgba(Drw *drw, Clr *dest, DrwColorF c)
{
	if (!drw || !dest)
		return;
	XRenderColor rc;
	rc.red = (unsigned short)(c.r * 65535.0f);
	rc.green = (unsigned short)(c.g * 65535.0f);
	rc.blue = (unsigned short)(c.b * 65535.0f);
	rc.alpha = (unsigned short)(c.a * 65535.0f);
	if (!XftColorAllocValue(drw->dpy, DefaultVisual(drw->dpy, drw->screen),
		DefaultColormap(drw->dpy, drw->screen), &rc, dest))
		die("error, cannot allocate rgba color");
}
